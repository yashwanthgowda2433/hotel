...
class Customer_model extends CI_Model
{
     ...
     */
    function addedNewCustomer($params=[])
    {

        if($this->existingCustomer($params))
        {
            if(!empty($params['email']))
            {
                $this->db->where('customerEmail', $params['email']);
            }
            if(!empty($params['phone_no']))
            {
                $this->db->where('customerPhone', $params['phone_no']);
            }
            $data = $this->db->get('ldg_customer');
            if($data->num_rows()>0)
            {
                $data = $data->row();
            ...
            {
                $this->db->set('customerName', $params['name']);
            }
            if(!empty($params['email']))
            {
                $this->db->set('customerEmail', $params['email']);
            }
            if(!empty($params['phone_no']))
            {
                $this->db->set('customerPhone', $params['phone_no']);
            }
            $data = $this->db->insert('ldg_customer');
    ...
    function existingCustomer($params=[])
    {
        if(!empty($params['email']))
        {
            $this->db->where('customerEmail', $params['email']);
